...
extern Memb_func* memb_func;
 ...
 "trap0_CaT", _hoc_trap0,
 0, 0
};
#define trap0 trap0_CaT
 extern double trap0( double , double , double , double );
 /* declare global and static user variables */
#define binf binf_CaT
 double binf = 0;
#define cinf cinf_CaT
 double cinf = 0;
#define q10 q10_CaT
 double q10 = 2;
#define taub taub_CaT
 double taub = 0;
#define tauc tauc_CaT
 double tauc = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 CaT /Users/dis006/Box Sync/T6 BP NEURON SIM/V3/Calcium_NMODL/MahapatraEtAl2018/x86_64/CaT.mod\n");
...
static int _reset;
static char *modelname = "T-type calcium  channel  ";
...
static int  rates (  double _lv ) {
  ...
  _lqt = pow( q10 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   binf = 1.0 / ( 1.0 + exp ( ( vhfa - _lv ) / sla ) ) ;
   cinf = 1.0 / ( 1.0 + exp ( ( _lv - vhfi ) / sli ) ) ;
   taub = mt * _lqt * ( 0.45 + ( 3.9 / ( 1.0 + pow( ( ( _lv + 66.0 ) / 26.0 ) , 2.0 ) ) ) ) ;
   tauc = ht * _lqt * ( 150.0 - ( 150.0 / ( ( 1.0 + exp ( ( _lv - 417.43 ) / 203.18 ) ) * ( 1.0 + exp ( - ( _lv + 61.11 ) / 8.07 ) ) ) ) ) ;
     return 0; }
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ica = gcat * pow( b , x ) * pow( c , y ) * ( v - eca ) ;
   icat = ica ;
   }
 _current += ica;
 ...
 v=_v;
{
  eca = _ion_eca;
 { error =  states();
 if(error){fprintf(stderr,"at line 85 in file CaT.mod:\n	    \n"); nrn_complain(_p); abort_run(error);}
 } }}
