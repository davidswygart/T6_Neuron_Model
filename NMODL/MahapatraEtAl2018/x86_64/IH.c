...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 IH /Users/dis006/Box Sync/T6 BP NEURON SIM/V3/Calcium_NMODL/MahapatraEtAl2018/x86_64/IH.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Inward-rectifying channel ";
...
/*CVODE*/
 ...
 {
   rates ( _threadargscomma_ v ) ;
    e = e + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / etau)))*(- ( ( ( einf ) ) / etau ) / ( ( ( ( - 1.0 ) ) ) / etau ) - e) ;
   }
  return 0;
}
...
static int  rates (  double _lv ) {
 ...
 _lqt = pow( q10 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   einf = 1.0 / ( 1.0 + exp ( ( _lv - vhfa ) / slp ) ) ;
   etau = rt * _lqt * ( 14.617 / ( 1.0 + exp ( ( 49.042 + _lv ) / tslp ) ) ) ;
    return 0; }
...
static void _initlists() {
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(e) - _p;  _dlist1[0] = &(De) - _p;
_first = 0;
}
