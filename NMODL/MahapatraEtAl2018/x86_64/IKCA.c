...
extern Memb_func* memb_func;
 ...
 "rates_IKCA", _hoc_rates,
 "trap0_IKCA", _hoc_trap0,
 0, 0
};
#define trap0 trap0_IKCA
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 IKCA /Users/dis006/Box Sync/T6 BP NEURON SIM/V3/Calcium_NMODL/MahapatraEtAl2018/x86_64/IKCA.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Calcium activated Potassium channel (IK)";
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    ca_i = ca_i + (1. - exp(dt*(( - ( 1.0 ) ) / tau)))*(- ( ( - ( ( - ca0 ) ) ) / tau ) / ( ( - ( 1.0 ) ) / tau ) - ca_i) ;
   rates ( _threadargscomma_ v , ca_i ) ;
...
static int  rates ( _threadargsprotocomma_ double _lv , double _lc ) {
 ...
 _lqt = pow( q10 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   linf = vha - 152.0 + ( 59.2 * exp ( - .09 * _lc * 1e3 ) ) + ( 96.7 * exp ( - .47 * _lc * 1e3 ) ) ;
   linf = min / ( 1.0 + exp ( ( linf - _lv ) / ( slp ) ) ) ;
   taul = mt * 5.0 * ( 17.0 / ( 1.0 + ( pow( ( ( _lv + 20.52 ) / 35.0 ) , 2.0 ) ) ) ) ;
   minf = 0.32 / ( 1.0 + exp ( ( _lv - cvh ) / ckh ) ) ;
   taum = ht * _lqt * ( 0.5 * ( 90.9699 * ( 1.0 - ( 1.0 / ( 1.0 + exp ( ( _lv + 13.9629 ) / 45.3782 ) ) ) * ( 1.0 / ( 1.0 + exp ( - ( _lv + 9.49866 ) / 3.3945 ) ) ) ) ) ) ;
    return 0; }
